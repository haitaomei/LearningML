...
# JAX is told how to shard the model's parameters (weights) across the device mesh 
# using PartitionSpec (aliased as P). A PartitionSpec
# defines how each dimension of a tensor is mapped to the mesh axes.
#
# Let's decode a rule: (("multi_head_attention", "...", "w"), P("data", "model")).
# This means that for a weight tensor w inside a multi_head_attention module, its first dimension 
# is sharded across the "data" axis of the mesh, and its second dimension is sharded across 
# the "model" axis. Since the mesh is 1x8, this effectively splits the tensor into
# 8 pieces along its second dimension, placing one piece on each of the 8 devices.

# 1 Input X (Replicated on Dev0 & Dev1)
# 2       |
# 3       |
# 4 [ FFN Block across 2 devices ]
# 5 ---------------------------------------------------------------------
# 6       |                                     |
# 7     Dev0                                  Dev1
# 8       |                                     |
# 9 (Layer 1: Column-sharded weights)             (Layer 1: Column-sharded weights)
# 10       |                                     |
# 11   X @ W1_col0 -> A0                       X @ W1_col1 -> A1
# 12       |                                     |
# 13    GELU(A0) -> B0                        GELU(A1) -> B1
# 14       |                                     |
# 15 (Layer 2: Row-sharded weights)                (Layer 2: Row-sharded weights)
# 16       |                                     |
# 17  B0 @ W2_row0 -> Y0                      B1 @ W2_row1 -> Y1
# 18       |                                     |
# 19       +------------ all-reduce (sum) -----------+
# 20                           |
# 21                           |
# 22                  Y = Y0 + Y1 (Result is now replicated on Dev0 & Dev1)
# 23                           |
# 24                           |
# 25            To next part of the model (e.g., Attention block or next layer)
...
class TransformerConfig:
    ...
    def get_memory_sharding(self):
        # This specifies that the Key (k) and Value (v) tensors in the cache 
        # are sharded across both the "data" and "model" axes, just like the
        # model weights.
#   Given the model's 8-way tensor parallelism, the KV cache is also sharded (split) across the 8 devices. Let's visualize how that
#   works.

#   1. High-Level Structure

#   First, the model doesn't have just one KV cache. It has a separate one for each of its 64 decoder layers. The top-level Memory object
#   is essentially a list of 64 KVMemory objects.

#     1 [ Overall Memory Structure ]
#     2 +-----------------------------------------------------------------+
#     3 | Memory object                                                   |
#     4 |                                                                 |
#     5 |   layers = [                                                    |
#     6 |     KVMemory_Layer0,  // Cache for Layer 0                      |
#     7 |     KVMemory_Layer1,  // Cache for Layer 1                      |
#     8 |     ...                                                         |
#     9 |     KVMemory_Layer63  // Cache for Layer 63                     |
#    10 |   ]                                                             |
#    11 +-----------------------------------------------------------------+

#   2. Single Layer KV Cache

#   Now, let's zoom into a single KVMemory object for one layer. It contains the tensors that store the Keys and Values, plus a step
#   counter to track the current length of the sequence.

#    1       |
#    2       | Zooms into a single layer's cache
#    3       v
#    4 [ KVMemory for a Single Layer (e.g., Layer i) ]
#    5 +-----------------------------------------------------------------+
#    6 |   k: jax.Array (The Key cache tensor)                           |
#    7 |   v: jax.Array (The Value cache tensor)                         |
#    8 |   step: jax.Array (Shape: [batch_size]) -> e.g., [15] means 15 tokens are cached |
#    9 +-----------------------------------------------------------------+

#   3. Sharding the K and V Tensors

#   This is where the parallelism becomes visible. The large k and v tensors are sharded across the 8 devices. The grok-1 model has 8
#   Key/Value heads (num_kv_heads=8), which maps perfectly to the 8-way model parallelism.

#   The sharding rule P(data_axis, model_axis) splits the num_kv_heads dimension across the "model" axis of the device mesh.

#   Here is how the Key tensor is distributed. The Value tensor (v) is distributed in exactly the same way.

#     1       |
#     2       | Zooms into the sharding of the 'k' tensor
#     3       v
#     4 [ Sharding of the Key Cache Tensor 'k' across 8 devices ]
#     5 
#     6 Tensor Shape: [batch_size, sequence_len, num_kv_heads, head_size]
#     7 Grok-1 Config: batch=B, seq_len=8192, num_kv_heads=8, head_size=128
#     8 Sharding Spec: P("data", "model") -> effectively P(None, "model") on a 1x8 mesh
#     9 
#    10 +----------------------------------+----------------+----------------+-----+----------------+
#    11 |              Dev0                |      Dev1      |      Dev2      | ... |      Dev7      |
#    12 +----------------------------------+----------------+----------------+-----+----------------+
#    13 |                                  |                |                |     |                |
#    14 | k_tensor[:, :, 0, :]             | k_tensor[:, :, 1, :] | k_tensor[:, :, 2, :] | ... | k_tensor[:, :, 7, :] |
#    15 | (Batch B, Seq 8192, Head 0, Dim 128) | (Head 1)       | (Head 2)       |     | (Head 7)       |
#    16 |                                  |                |                |     |                |
#    17 +----------------------------------+----------------+----------------+-----+----------------+
#    18 ^                                  ^                ^                ^     ^                ^
#    19 |                                  |                |                |     |                |
#    20 +--- Each device holds the data for one Key/Value head for all sequences in the batch. ---+

#   In summary:

#    - The full KV cache is pre-allocated to hold up to the maximum sequence length (8192 tokens).
#    - Each of the 8 devices is responsible for storing the Keys and Values for exactly one of the 8 attention heads.
#    - When the model performs attention, each device works on its local slice of the KV cache in parallel. This is highly efficient as it
#      minimizes data transfer between devices.
        return Memory(
            layers=[
                KVMemory(
...
class DecoderLayer(hk.Module):
    ...
    def __call__(
        ...
        # This ensures that the output of one layer remains distributed across 
        # devices as it's passed to the next, avoiding memory bottlenecks
        # on a single device.
